...
try:
    response = requests.get("https://pncp.gov.br/api/pncp/v3/api-docs", timeout=10)
    if response.status_code == 200:
        # Save the raw JSON response
        with open("api_docs_raw.json", "w", encoding="utf-8") as f:
            json.dump(response.json(), f, ensure_ascii=False, indent=2)
        print("API documentation saved to api_docs_raw.json")
